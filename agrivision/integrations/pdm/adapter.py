...
def _extract_risk_entries(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in graph if isinstance(graph, list) else []:
        ...
        for risk in risks:
            ...
            entries.append(
                {
                    'timestamp': risk.get('saref:hasTimestamp') or risk.get('timestamp') or '',
                    'risk_level': str(risk.get('ocsm:hasRiskLevel') or risk.get('risk_level') or ''),
                    'model_id': item.get('@id') or item.get('id') or '',
                    'eppo_code': item.get('fsm:eppoCode') or item.get('eppo_code') or '',
                    'description': item.get('foodie:description') or item.get('description') or '',
                }
            )
    return entries


def _summarize_risks(entries: list[dict[str, Any]], model: dict[str, Any]) -> tuple[str | None, list[str], str, dict[str, Any]]:
    if not entries:
        return None, ['PDM service returned no risk entries for the selected model and parcel.'], model.get('default_recommendation', ''), {
            'counts': {}, 'latest_timestamp': None, 'highest_timestamp': None
        }
    counts: dict[str, int] = {}
    highest = None
    latest_ts = None
    for item in entries:
        ...
        if ts and (latest_ts is None or str(ts) > str(latest_ts)):
            latest_ts = str(ts)
        if highest is None or RISK_ORDER.get(level.lower(), 0) > RISK_ORDER.get(str(highest.get('risk_level')).lower(), 0):
            highest = item
    risk_level = str(highest.get('risk_level')).title() if highest else None
    reasons = [f"Remote PDM returned {counts.get('High', 0)} High, {counts.get('Medium', 0)} Medium, {counts.get('Low', 0)} Low risk observations."]
...
def collect_pdm_snapshot(
    ...
    *,
    ...
    raw_payload = client.calculate_risk_index(
        resource_id=base_summary['dataset_upload_id'],
        parcel_id=base_summary['remote_parcel_id'],
        model_ids=[base_summary['remote_model_id']],
        high_only=False,
    )
    entries = _extract_risk_entries(raw_payload)
    risk_level, reasons, recommendation, stats = _summarize_risks(entries, resolved_model)

    base_summary['status'] = 'success' if entries else 'partial'
    base_summary['warning_state'] = None if entries else 'no_remote_results'
    ...
    base_summary['notes'] = [
        ...
        f"Dataset upload id: {base_summary['dataset_upload_id']}" if base_summary['dataset_upload_id'] else 'Dataset upload id unavailable.',
    ]
    if bootstrap.get('dataset_upload'):
        base_summary['notes'].append('Weather dataset upload was attempted before risk-index execution.')
