...
def test_build_package_includes_run_and_quality_artifacts(tmp_path: Path) -> None:
    ...
    report.write_text('<html><head></head><body><img src="vegetation_index/vegetation_index_color.png" /></body></html>', encoding='utf-8')
    vegetation_index_dir = workspace.vegetation_index_output
    vegetation_index_dir.mkdir(parents=True)
    (vegetation_index_dir / 'metadata.json').write_text(json.dumps({'quality': 'ok'}), encoding='utf-8')
    (vegetation_index_dir / 'grid_metadata.json').write_text(json.dumps({'grid': {}}), encoding='utf-8')
    (vegetation_index_dir / 'vegetation_index_grid_cells.csv').write_text('cell,value\nA1,1\n', encoding='utf-8')
    (vegetation_index_dir / 'vegetation_index_grid_overlay.png').write_bytes(b'png')
    (vegetation_index_dir / 'vegetation_index_color.png').write_bytes(b'png')
    service.update_status(
        record.run_id,
        status='completed',
        outputs={'report_html': str(report), 'vegetation_index_metadata': str(vegetation_index_dir / 'metadata.json')},
    )
    ...
    with zipfile.ZipFile(package) as archive:
        ...
        assert 'report-assets/vegetation_index/vegetation_index_color.png' in names
